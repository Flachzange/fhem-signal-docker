...
RUN wget https://github.com/adoptium/temurin21-binaries/releases/download/jdk-21.0.9%2B10/OpenJDK21U-jdk_x64_linux_hotspot_21.0.9_10.tar.gz
RUN tar zxf OpenJDK21U-jdk_x64_linux_hotspot_21.0.9_10.tar.gz
RUN mv jdk* java
RUN rm OpenJDK21U-jdk_x64_linux_hotspot_21.0.9_10.tar.gz
...
RUN wget -qN https://github.com/AsamK/signal-cli/releases/download/v0.13.22/signal-cli-0.13.22.tar.gz -O signal-cli-0.13.22.tar.gz
RUN tar zxf signal-cli-0.13.22.tar.gz
RUN mv signal-cli-0.13.22  /opt/signal
...
RUN wget -qN https://github.com/exquo/signal-libs-build/releases/download/libsignal_v0.86.2/libsignal_jni.so-v0.86.2-x86_64-unknown-linux-gnu.tar.gz
RUN tar zxf libsignal_jni.so-v0.86.2-x86_64-unknown-linux-gnu.tar.gz
RUN zip -u /opt/signal/lib/libsignal-client-*.jar libsignal_jni.so

RUN rm -f signal-cli-0.13.22.tar.gz libsignal_jni.so
